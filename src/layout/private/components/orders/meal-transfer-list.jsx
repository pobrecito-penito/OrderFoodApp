...
//COMPONENT
const MealTransferList = ({restaurantId,pollId}) => {

    const [meals,setMeals] = useState([]);
    const [left, setLeft] = React.useState([]);
    useEffect(() => {
        findData('meal','').then(res => {
            let allMeals = res.data.data;
            setMeals(allMeals.filter(el => el.restaurantId === restaurantId));
            setLeft(allMeals.filter(el => el.restaurantId === restaurantId));
        })
    },[restaurantId])
    ...
    const handleAllRight = () => {
        let available = left.filter(el => el.available === true);
        setRight(right.concat(available));
        
        let unavailable = left.filter(el => el.available === false);
        setLeft(unavailable);
    };
    ...
    const customList = (items) => (
        ...
        <List dense component="div" role="list">
            ...
            return (
                <ListItem key={value.id} role="listitem" button onClick={value.available ? handleToggle(value) : noToggle()}>
                <ListItemIcon>
                    <Checkbox
                    disabled={!value.available}
                    checked={value.available ? checked.indexOf(value) !== -1 : false}
                    tabIndex={-1}
        ...
        return(
            <ListItem key={el.id} button>
              ...
              <ListItemText id={labelId} primary={el.price + ' din'} />
                <FormControl style={{width: "60px"}} type="number" step={1} defaultValue={'1'} onChange={(e) => objects.find(element => element.mealId === el.id).quantity = e.target.value} />
...
export default MealTransferList